...
"""Copy the Stencil Islands extension into Inkscape's per-user extensions folder.
...
# Every file Inkscape needs at runtime: the .inx descriptor, the entry
# script, and the pure-Python helper modules it imports.
FILES = (
    "stencil_islands.inx",
    "stencil_islands.py",
    "si_geometry.py",
...
def main():
    parser = argparse.ArgumentParser(
        description="Install the Stencil Islands Inkscape extension."
    )
    parser.add_argument(
        "--uninstall", action="store_true", help="remove installed files"
    )
    parser.add_argument(
        "--path", action="store_true", help="print the target folder and exit"
    )
    ...
    if args.uninstall:
        ...
        print(
            "Uninstalled ({} file(s)).".format(removed)
            if removed
    ...
    print(
        "\nDone. Restart Inkscape, then use Extensions > Stencil > "
        "Find Islands (Stencil Bridges)."
    )
